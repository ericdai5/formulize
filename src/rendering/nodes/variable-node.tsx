...
const VariableNode = observer(({ data }: { data: VariableNodeData }) => {
  ...
  const handleClick = () => {
    if (isSetVariable) {
      // Toggle label display for set variables
      const currentDisplay = variable?.labelDisplay || "value";
      const newDisplay = currentDisplay === "value" ? "none" : "value";
      computationStore.setVariableProperty(varId, "labelDisplay", newDisplay);
    }
  };
  ...
  const cursor = isSetVariable ? "pointer" : 
