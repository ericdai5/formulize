 ...
 */
const InlineFormulaInner = observer(({ id, scale = 1 }: { id: string; scale?: number }) => {
  ...
  const renderFormula = useCallback(async () => {
    ...
    try {
      ...
      try {
        ...
        console.warn("InlineFormula: LaTeX processing failed, using raw latex");
  ...
  }, [id, getFormulaLatex, mathJaxLoaded]);
  ...
  ({ id, className = "", style = {}, scale = 1 }) => {
    ...
    return (
      <span
        ...
        style={{ display: "inline", ...style }}
      >
        <InlineFormulaInner id={id} scale={scale} />
      </span>
    );
  }
);
